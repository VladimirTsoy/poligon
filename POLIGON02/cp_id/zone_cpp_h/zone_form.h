...
#include "zone_convertor.h"
#include "zone_painter.h"

// форма
class zone_form : public QWidget
{
    Q_OBJECT
public:
    // констуктор
    zone_form(QString _xml_full_dir_path, QString _xml_file_name,
    ...
    QPushButton * ret_p_b_show_map();                                           // вернуть кнопку Показать карту
    xml_xsd_xsl_frame * ret_ptr_xml_xsd_xsl_frame();                            // вернуть фрейм xml-/xsd-/xsl-документов
    zone_table_data * ret_ptr_table_data();                                     // вернуть данные таблицы
    zone_table_view * ret_ptr_table_view();                                     // вернуть представление таблицы
    zone_convertor * ret_ptr_convertor();                                       // вернуть конвертор
    ...
    void set_p_b_show_map(QPushButton * _p_b_show_map);                         // установить кнопку Показать карту
    void set_ptr_xml_xsd_xsl_frame(xml_xsd_xsl_frame * _ptr_xml_xsd_xsl_frame); // установить фрейм xml-/xsd-/xsl-документов
    void set_ptr_table_data(zone_table_data * _ptr_table_data);                 // установить данные таблицы
    void set_ptr_table_view(zone_table_view * _ptr_table_view);                 // установить представление таблицы
    void set_ptr_convertor(zone_convertor * _ptr_convertor);                    // установить конвертор
...
private:
    ...
    QPushButton * p_b_show_map;                // кнопка Показать карту

    xml_xsd_xsl_frame * ptr_xml_xsd_xsl_frame; // фрейм xml-/xsd-/xsl-документов
    zone_table_data * ptr_table_data;          // данные таблицы
    zone_table_view * ptr_table_view;          // представление таблицы
    zone_convertor * ptr_convertor;            // конвертор

    zone_painter * ptr_zone_painter;
